...
fn print_sim_rec(digits: &str, n: usize) {
...
fn print_sim_dir(input: &str, n: usize) {
...
impl KeypadMap {
    ...
    fn input_count(&self, input: &str) -> Option<usize> {
        input.split_inclusive('A').fold(Some(0), |acc, part| {
            acc.and_then(|sum| Some(sum + self.0.get(part)?))
        })
    }
